...
def get_completion(message, agent, funcs, thread):
    ...
    defined within a func parameter. Each funtion in the array must implement a `run()` method that returns the outputs.
    ...
    while True:
        ...
        if run.status == "requires_action":
            ...
            for tool_call in tool_calls:
                ...
                wprint('\033[31m' + str(tool_call.function), '\033[0m]')
                tool_outputs.append({"tool_call_id": tool_call.id, "outputs": output})
...
# Agent responsible for creating and executing python files.
from typing import List
from pydantic import Field
from instructor import OpenAISchema

class ExecutePyFile(OpenAISchema):
    """Run exesting python file from local disc."""
    file_name: str = Field(
...
class File(OpenAISchema):
    """
    Python file with an appropriate name, containing code that can be saved and executed locally ata later time. This environment has access to all python packages and the internet.
    """
    chain_of_thought: str = Field(..., description="Think step by step to determine the correct actions that are neeeded  to be taken in order to complete tasks.")
...
class SendMessage(OpenAISchema):
    ...
    message: str = Field(..., description="Specify the task required for the recipient agent to complete. Focus instead on clarifying what the task entails, rather than providing detailed instructions.")
...
user_proxy = client.beta.assistants.create(
    ...
    instructions= """"As a user proxy agent, your reponsibility is to streamline the dialogue between the user and specialized agents within this group chat.
    ...
    Please do not respond to the user until the task is complete, an error has been reported by the relevant agent, or you are certain of your reponse.""",
    model="gpt-4-1106-preview",
    tools=[
        {
...
while True:
    user_messsage = input("User: ")
    message = get_completion(user_messsage, user_proxy, user_proxy_tools, thread)
